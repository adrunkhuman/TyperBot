...
class TestSchemaMigration:
    ...
    @pytest.mark.asyncio
    async def test_initialize_rejects_existing_fixtures_without_season_migration(
        self, temp_db_path
    ):
        async with aiosqlite.connect(temp_db_path) as conn:
            await conn.execute(
            ...
            await conn.execute(
                "INSERT INTO guild_config (guild_id, admin_role_id, league_channel_id) VALUES ('111111', 'role-1', 'channel-1')"
            )
            await conn.execute(
                "INSERT INTO guild_config (guild_id, admin_role_id, league_channel_id) VALUES ('222222', 'role-2', 'channel-2')"
            )
            await conn.execute(
                "INSERT INTO fixtures (id, guild_id, week_number, games, deadline, status) VALUES (1, '111111', 1, 'A - B', ?, 'closed')",
                (datetime.now(UTC).isoformat(),),
            )
            await conn.execute(
                "INSERT INTO fixtures (id, guild_id, week_number, games, deadline, status) VALUES (2, '111111', 2, 'C - D', ?, 'open')",
                (datetime.now(UTC).isoformat(),),
            )
            await conn.execute(
                "INSERT INTO fixtures (id, guild_id, week_number, games, deadline, status) VALUES (3, '222222', 1, 'E - F', ?, 'open')",
                (datetime.now(UTC).isoformat(),),
            )
            await conn.commit()
        ...
        with pytest.raises(RuntimeError, match="fixtures.season_id is missing"):
            await db.initialize()

    @pytest.mark.asyncio
    async def test_initialize_rejects_cross_guild_fixture_season(self, temp_db_path):
        async with aiosqlite.connect(temp_db_path) as conn:
            await conn.execute(
                """
                CREATE TABLE seasons (
            ...
            await conn.execute(
                "INSERT INTO seasons (id, guild_id, name, status) VALUES (1, '222222', 'Wrong Guild', 'active')"
            )
            await conn.execute(
                "INSERT INTO seasons (id, guild_id, name, status) VALUES (2, '111111', 'Current Season', 'active')"
            )
            await conn.execute(
                "INSERT INTO guild_config (guild_id, admin_role_id, league_channel_id, active_season_id) VALUES ('111111', 'role-1', 'channel-1', 1)"
            )
            await conn.execute(
                "INSERT INTO fixtures (id, guild_id, week_number, games, deadline, status, season_id) VALUES (1, '111111', 1, 'A - B', ?, 'open', 1)",
                (datetime.now(UTC).isoformat(),),
            )
            await conn.commit()

        db = Database(temp_db_path)
        with pytest.raises(RuntimeError, match="fixtures.season_id has empty or cross-guild rows"):
            await db.initialize()

    @pytest.mark.asyncio
    async def test_initialize_rejects_multiple_active_seasons_per_guild(self, temp_db_path):
        async with aiosqlite.connect(temp_db_path) as conn:
            await conn.execute(
                """
                CREATE TABLE seasons (
            ...
            await conn.execute(
                "INSERT INTO seasons (id, guild_id, name, status) VALUES (1, '111111', 'Old Active', 'active')"
            )
            await conn.execute(
                "INSERT INTO seasons (id, guild_id, name, status) VALUES (2, '111111', 'New Active', 'active')"
            )
            await conn.execute(
                "INSERT INTO fixtures (id, guild_id, week_number, games, deadline, status, season_id) VALUES (1, '111111', 1, 'A - B', ?, 'open', 1)",
                (datetime.now(UTC).isoformat(),),
        ...
        db = Database(temp_db_path)
        with pytest.raises(RuntimeError, match="multiple active seasons exist"):
            await db.initialize()
