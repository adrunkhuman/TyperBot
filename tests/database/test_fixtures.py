...
class TestGetMaxWeekNumber:
    ...
    async def test_get_max_week_number_empty_db(self, temp_db_path):
        ...
        result = await db.fixtures.get_max_week_number("111111")
        assert result == 0
    ...
    async def test_get_max_week_number_with_fixtures(self, temp_db_path):
        ...
        await db.fixtures.create_fixture("111111", 3, ["Team C - Team D"], datetime.now(UTC))
        await db.fixtures.create_fixture("111111", 5, ["Team E - Team F"], datetime.now(UTC))
    ...
    async def test_get_max_week_number_closed_fixtures(self, temp_db_path):
        ...
        fixture_id = await db.fixtures.create_fixture(
            "111111", 10, ["Team A - Team B"], datetime.now(UTC)
        )
        ...
        await db.fixtures.create_fixture("111111", 5, ["Team C - Team D"], datetime.now(UTC))
    ...
    async def test_get_max_week_number_is_active_season_scoped(self, temp_db_path):
        db = Database(temp_db_path)
        await db.initialize()
        await db.fixtures.create_fixture("111111", 10, ["Team A - Team B"], datetime.now(UTC))
        await start_new_active_season(temp_db_path, "111111")

        assert await db.fixtures.get_max_week_number("111111") == 0

        await db.fixtures.create_fixture("111111", 1, ["Team C - Team D"], datetime.now(UTC))

        assert await db.fixtures.get_max_week_number("111111") == 1
...
class TestOpenFixturesQueries:
    ...
    async def test_get_open_fixtures_returns_all_open_ordered(self, temp_db_path):
        ...
        fixture_week_2 = await db.fixtures.create_fixture(
            "111111", 2, ["Team C - Team D"], datetime.now(UTC)
        )
        fixture_week_1 = await db.fixtures.create_fixture(
            "111111", 1, ["Team A - Team B"], datetime.now(UTC)
        )
        fixture_week_3 = await db.fixtures.create_fixture(
        ...
        await db.fixtures.create_fixture("guild-2", 1, ["Other A - Other B"], datetime.now(UTC))
    ...
    async def test_get_open_fixture_by_week_ignores_closed_fixtures(self, temp_db_path):
        ...
        closed_fixture_id = await db.fixtures.create_fixture(
            "111111", 8, ["Team C - Team D"], datetime.now(UTC)
        )
        await db.scores.save_scores(closed_fixture_id, [])

        open_fixture = await db.fixtures.get_open_fixture_by_week("111111", 7)
        closed_fixture = await db.fixtures.get_open_fixture_by_week("111111", 8)
    ...
    async def test_week_and_recent_fixture_queries_are_guild_scoped(self, temp_db_path):
        ...
        guild_one_week = await db.fixtures.create_fixture(
            "111111", 1, ["Team A - Team B"], datetime.now(UTC)
        )
        guild_two_week = await db.fixtures.create_fixture(
            "guild-2", 1, ["Team C - Team D"], datetime.now(UTC)
        )

        assert (await db.fixtures.get_fixture_by_week("111111", 1))["id"] == guild_one_week
        assert (await db.fixtures.get_fixture_by_week("guild-2", 1))["id"] == guild_two_week
        assert [fixture["id"] for fixture in await db.fixtures.get_recent_fixtures("111111")] == [
            guild_one_week
        ]
        assert await db.fixtures.get_fixture_by_id(guild_two_week, "111111") is None
    ...
    async def test_delete_fixture_can_require_guild_ownership(self, temp_db_path):
        ...
        fixture_id = await db.fixtures.create_fixture(
            "guild-2", 1, ["Team A - Team B"], datetime.now(UTC)
        )

        assert await db.fixtures.delete_fixture(fixture_id, "111111") is False
        assert await db.fixtures.get_fixture_by_id(fixture_id, "guild-2") is not None

        assert await db.fixtures.delete_fixture(fixture_id, "guild-2") is True
        assert await db.fixtures.get_fixture_by_id(fixture_id, "guild-2") is None
    ...
    async def test_get_prediction_requires_fixture_guild_ownership(self, temp_db_path):
        ...
        await db.predictions.save_prediction(fixture_id, "user-1", "User One", ["2-1"], False)

        assert await db.predictions.get_prediction(fixture_id, "user-1", "111111") is None
        assert await db.predictions.get_prediction(fixture_id, "user-1", "guild-2") is not None
    ...
    async def test_pending_partial_predictions_are_guild_scoped(self, temp_db_path):
        ...
        guild_one_pending = await db.predictions.get_pending_partial_predictions("111111")
        guild_two_pending = await db.predictions.get_pending_partial_predictions("guild-2")
    ...
    async def test_create_next_fixture_allocates_incrementing_weeks(self, temp_db_path):
        ...
        fixture_one_id, week_one = await db.fixtures.create_next_fixture(
            "111111",
            ["Team A - Team B"],
            datetime.now(UTC),
        )
        fixture_two_id, week_two = await db.fixtures.create_next_fixture(
            "111111",
            ["Team C - Team D"],
            datetime.now(UTC),
        )

        fixture_one = await db.fixtures.get_fixture_by_id(fixture_one_id, "111111")
        fixture_two = await db.fixtures.get_fixture_by_id(fixture_two_id, "111111")
    ...
    async def test_created_fixtures_store_guild_ownership(self, temp_db_path):
        ...
        fixture = await db.fixtures.get_fixture_by_id(fixture_id, "guild-2")
    ...
    async def test_create_fixture_rejects_missing_guild_id(self, temp_db_path):
        ...
        with pytest.raises(ValueError, match="guild_id is required"):
            await db.fixtures.create_fixture("", 1, ["Team A - Team B"], datetime.now(UTC))
    ...
    async def test_create_next_fixture_rejects_missing_guild_id(self, temp_db_path):
        ...
        with pytest.raises(ValueError, match="guild_id is required"):
            await db.fixtures.create_next_fixture("", ["Team A - Team B"], datetime.now(UTC))
    ...
    async def test_create_next_fixture_allocates_weeks_per_guild(self, temp_db_path):
        ...
        _, guild_one_week = await db.fixtures.create_next_fixture(
            "111111",
            ["Team A - Team B"],
            datetime.now(UTC),
        )
        _, guild_two_week = await db.fixtures.create_next_fixture(
            "guild-2",
            ["Team C - Team D"],
            datetime.now(UTC),
        )
        guild_one_second_id = await db.fixtures.create_fixture(
            "111111",
            2,
            ["Team E - Team F"],
            datetime.now(UTC),
        ...
        assert await db.fixtures.get_max_week_number("111111") == 2
        assert await db.fixtures.get_max_week_number("guild-2") == 9

        guild_one_second = await db.fixtures.get_fixture_by_id(guild_one_second_id, "111111")
...
class TestCreateNextFixtureConcurrency:
    ...
    async def test_concurrent_calls_allocate_distinct_week_numbers(self, temp_db_path):
        ...
        created = await asyncio.gather(
            db.fixtures.create_next_fixture("111111", ["A - B"], datetime.now(UTC)),
            db.fixtures.create_next_fixture("111111", ["C - D"], datetime.now(UTC)),
        )
        ...
        fixtures = [
            await db.fixtures.get_fixture_by_id(fixture_id, "111111") for fixture_id in fixture_ids
        ]
...
class TestRowToFixture:
    ...
    async def test_empty_games_column_returns_empty_list(self, temp_db_path):
        ...
        db = Database(temp_db_path)
        await db.initialize()
        season = await db.seasons.get_or_create_active_season("111111")
        ...
        fixture = await db.fixtures.get_current_fixture("111111")
        assert fixture is not None
