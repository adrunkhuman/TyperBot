...
class TestScores:
    ...
    async def test_result_correction_recalculates_with_active_season_rules(self, temp_db_path):
        ...
        await db.scores.recalculate_fixture_scores(fixture_id)

        await db.results.save_results_with_recalc(fixture_id, ["2-0"])

        scores = await db.scores.get_scores_for_fixture(fixture_id)
    ...
    async def test_prediction_replacement_recalculates_with_active_season_rules(self, temp_db_path):
        ...
        updated = await db.predictions.admin_update_prediction_with_recalc(
    ...
    async def test_waiver_toggle_recalculates_with_active_season_rules(self, temp_db_path):
        ...
        fixture_id = await db.fixtures.create_fixture("111111", 1, ["A - B"], datetime.now(UTC))
        await db.results.save_results(fixture_id, ["2-1"])
        await db.predictions.save_prediction(fixture_id, "user-1", "User One", ["2-1"], True)
        await db.scores.recalculate_fixture_scores(fixture_id)

        waived = await db.predictions.toggle_late_penalty_waiver_with_recalc(fixture_id, "user-1")
    ...
    async def test_partial_rejection_recalculates_with_active_season_rules(self, temp_db_path):
        ...
        rejected = await db.predictions.reject_partial_prediction_with_recalc(fixture_id, "partial")
    ...
    async def test_recalculate_fixture_scores_uses_active_season_rules(self, temp_db_path):
        ...
        await db.results.save_results(fixture_id, ["2-1", "1-1", "2-0"])
        await db.predictions.save_prediction(
            fixture_id,
            "user-1",
            "User One",
    ...
    async def test_late_prediction_uses_active_season_penalty_rule(self, temp_db_path):
        db = Database(temp_db_path)
        await db.initialize()
        await db.seasons.update_active_scoring_rules("111111", {"late_prediction_points": 1})
        fixture_id = await db.fixtures.create_fixture("111111", 1, ["A - B"], datetime.now(UTC))
        await db.results.save_results(fixture_id, ["2-1"])
        await db.predictions.save_prediction(fixture_id, "late", "Late User", ["2-1"], True)
        await db.predictions.save_prediction(fixture_id, "waived", "Waived User", ["2-1"], True)
        await db.predictions.set_late_penalty_waiver(fixture_id, "waived", True)
    ...
    async def test_scoring_rules_are_guild_isolated(self, temp_db_path):
        ...
        guild_one_fixture_id = await db.fixtures.create_fixture(
            "111111", 1, ["A - B"], datetime.now(UTC)
        )
        guild_two_fixture_id = await db.fixtures.create_fixture(
            "222222", 1, ["A - B"], datetime.now(UTC)
        )
        ...
        guild_one_scores = await db.scores.get_scores_for_fixture(guild_one_fixture_id)
        guild_two_scores = await db.scores.get_scores_for_fixture(guild_two_fixture_id)
    ...
    async def test_scoring_rule_changes_are_blocked_after_scores_exist(self, temp_db_path):
        db = Database(temp_db_path)
        await db.initialize()
        assert await db.seasons.active_season_has_scores("111111") is False
        fixture_id = await db.fixtures.create_fixture("111111", 1, ["A - B"], datetime.now(UTC))
        await db.results.save_results(fixture_id, ["2-1"])
        await db.predictions.save_prediction(fixture_id, "user-1", "User One", ["2-1"], False)
        await db.scores.recalculate_fixture_scores(fixture_id)

        assert await db.seasons.active_season_has_scores("111111") is True
    ...
    async def test_scoring_rule_change_block_is_guild_and_active_season_scoped(self, temp_db_path):
        db = Database(temp_db_path)
        await db.initialize()
        old_fixture_id = await db.fixtures.create_fixture("111111", 1, ["A - B"], datetime.now(UTC))
        ...
        await db.seasons.update_active_scoring_rules("111111", {"correct_outcome_points": 2})

        active_fixture_id = await db.fixtures.create_fixture(
            "111111", 1, ["E - F"], datetime.now(UTC)
        )
        await db.scores.save_scores(
            active_fixture_id,
            [
        ...
        with pytest.raises(ValueError, match="Cannot change scoring rules"):
            await db.seasons.update_active_scoring_rules("111111", {"wrong_outcome_points": 1})
    ...
    async def test_recalculate_fixture_scores_rolls_back_after_partial_write_failure(
        ...
        await db.predictions.save_prediction(
            fixture_id, "user-1", "User One", ["2-1", "1-1"], False
        )
        await db.results.save_results(fixture_id, ["2-1", "1-1"])
        await db.scores.save_scores(
            fixture_id,
            [
                {
    ...
    async def test_standings_order_by_points_tiebreakers_and_name(self, temp_db_path):
        ...
        standings = await db.scores.get_standings("111111")
    ...
    async def test_get_standings_for_season_returns_archived_season_scores(self, temp_db_path):
        ...
        standings = await db.scores.get_standings_for_season("111111", old_season["id"])
        wrong_guild_standings = await db.scores.get_standings_for_season("222222", old_season["id"])
    ...
    async def test_last_fixture_scores_are_active_season_scoped(self, temp_db_path):
        ...
        later_archived_fixture_id = await db.fixtures.create_fixture(
        ...
        last_fixture = await db.scores.get_last_fixture_scores("111111")
